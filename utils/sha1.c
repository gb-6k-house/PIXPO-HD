...
sha1_process_bytes (const void *buffer, size_t len, struct sha1_ctx *ctx)
{
  ...
  if (len >= 64)
    {
#define _STRING_ARCH_unaligned 64
...
/*参数分别是:计算摘要的key，key长度，需要计算摘要的消息内容，消息内容的长度，摘要结果*/
int
...
/*计算完毕摘要后，调用这个函数将摘要转为字符串，填到消息就可以了*/
