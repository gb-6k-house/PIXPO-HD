...
- (void)displayImage:(UIImage *)image {
    ...
    MMImageViewer *imageView = [[MMImageViewer alloc] initWithImage:image];
    ...
    deviceManager.imageView = self.imageView;
    //[deviceManager startVideo];
    DebugLog(@"PZPhotoView<==displayImage");
}
...
- (void)handleSwipe:(UISwipeGestureRecognizer *)gestureRecognizer {
    if (self.photoViewDelegate != nil) {
        [self.photoViewDelegate photoViewDidSwipe:self];
    }
    ...
    switch (gestureRecognizer.direction) {
        case UISwipeGestureRecognizerDirectionLeft:
            NSLog(@"Swipe left");
            [deviceManager.currentDevice send_ptz_cmd:2];
            break;
        case UISwipeGestureRecognizerDirectionRight:
            NSLog(@"Swipe right");
            [deviceManager.currentDevice send_ptz_cmd:3];
            
            break;
        case UISwipeGestureRecognizerDirectionUp:
            NSLog(@"Swipe Up");
            [deviceManager.currentDevice send_ptz_cmd:0];
            
            break;
        case UISwipeGestureRecognizerDirectionDown:
            NSLog(@"Swipe Down");
            [deviceManager.currentDevice send_ptz_cmd:1];
...
- (void)handleTabBarClick:(UITabBarItem *)tabBarItem {
    if (self.photoViewDelegate != nil) {
...
- (void)handleTwoFingerTap:(UIGestureRecognizer *)gestureRecognizer {
    ...
    CGFloat newScale = MAX([self zoomScale], self.minimumZoomScale);
    
    NSLog(@"handleTwoFingerTap==> [newscale:%f , zoomScale:[%f] miniScale[%f]]", newScale,[self zoomScale],self.minimumZoomScale);
...
- (CGRect)zoomRectForScale:(float)scale withCenter:(CGPoint)center {
    //NSLog(@"new scale:%f maxscale:%f, minscale:%f, frame[w:%f,h:%f]",scale,self.maximumZoomScale,self.minimumZoomScale, self.frame.size.width,self.frame.size.height);
